...
namespace c2p2::modules {
    ...
    class Base64 : public Module {
    public:
        [[nodiscard]] std::string get_id() const override { return "base64"; }
        [[nodiscard]] std::vector<std::string> get_supported_actions() const override {
            return {"encode", "decode"};
        }

        [[nodiscard]] DataBuffer help_text() const override {
            const std::string help = "Base64 Module:\n"
                               "Actions:\n"
                               "  encode: Encode input data to Base64.\n"
                               "  decode: Decode Base64 input data.\n"
                               "Parameters:\n"
